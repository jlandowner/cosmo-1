...
func ServiceAccountLogin(ctx context.Context, cfg *Config) error {
	ca, err := downloadInClusterCA() // TODO: cache
	if err != nil {
	...
	c := NewCosmoDashClient(httpClient, baseURL)
...
func downloadInClusterCA() ([]byte, error) {
	url := fmt.Sprintf("%s/%s", inClusterCAServerURL(), InClusterCAFile)
