	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := C2D_Workspace(tt.args.ws, tt.args.opts...); !reflect.DeepEqual(got.String(), tt.want.String()) {
				...
				t.Errorf("C2D_Workspace() = %v, want %v", *got, *tt.want)
			}
		})
	}
}
...
func TestC2D_NetworkRule(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := C2D_NetworkRule(tt.args.v); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("C2D_NetworkRule() = %v, want %v", got, tt.want)
			}
		})
	}
}
