...
func (b *TempKustomizeBuilder) SetBaseYAML(rawYAML string) *TempKustomizeBuilder {
	b.baseYAMLData = rawYAML
	b.Resources(b.baseYAMLFile)
	return b
}

func (b *TempKustomizeBuilder) Resources(fileNames ...string) *TempKustomizeBuilder {
...
func (b *TempKustomizeBuilder) Build(ctx context.Context) ([]byte, error) {
	...
	// save base yaml
	if err := os.WriteFile(filepath.Join(b.tmpDir, b.baseYAMLFile), []byte(b.baseYAMLData), 0644); err != nil {
		return nil, fmt.Errorf("failed to write file %s: %w", filepath.Join(b.tmpDir, b.baseYAMLFile), err)
	}
